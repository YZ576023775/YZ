...
public class UpdateNewMusicCrawlStart {
	 final static Logger logger = Logger.getLogger(UpdateNewMusicCrawlStart.class);
	 static int size;
	...
	public static void main(String[] args) {
		...
		Document document = null;
			try {
				document = reader.read(UpdateNewMusicCrawlStart.class.getClassLoader().getResourceAsStream("crawls.xml"));
		...
		for (int i = 0; i < size; i++) {
			...
			try {
				doc = reader.read(UpdateNewMusicCrawlStart.class.getClassLoader().getResourceAsStream(path));
...
class MyRunnable3 implements Runnable{
	...
	public void run() {
		...
		UpdateWebCrawler wc = new UpdateWebCrawler();
...
//		UrlTools urlTools = new UrlTools(MusicUrlRgx, MvUrlRgx, ArtistUrlRgx, AlbumUrlRgx);
		...
		while (true) {
			if (wc.notCrawlurlSet.isEmpty()
				...
				synchronized (UpdateNewMusicCrawlStart.class) {

					UpdateNewMusicCrawlStart.exitCount++;
					if (UpdateNewMusicCrawlStart.exitCount == UpdateNewMusicCrawlStart.size) {
						System.exit(1);
					}else {
						break;
					}
