(function(){
    ...
    // Init Canvas
    function initStages() {
        offsetX = (window.innerWidth-650)/2;
        offsetY = (window.innerHeight-550)/2;
...
//        form.style.left = offsetX+'px';
    ...
    function initText() {
        text = new createjs.Text("t", "65px 'Source Sans Pro'", "#eee");
    ...
    function initCircles() {
        ...
        for(var i=0; i<600; i++) {
            ...
            var y = window.innerHeight*2*Math.random();
    ...
    function addListeners() {
        input.onkeyup = function(event) {
               ...
               if (keyCode != 13&&keyCode != 38 && keyCode != 40) { //不是三个特殊键，可以搜索
		            if(textFormed) {
		                explode();
		                if(input.value != ''&&tmp!=input.value) {
		                    setTimeout(function() {
		                    	var s=input.value.toLocaleUpperCase();
		                        createText(s);
		                    }, 810);
		                    tmp=input.value;
		                } else {
